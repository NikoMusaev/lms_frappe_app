...
def _требовать_покрытие(занятие) -> None:
	"""Отказывает в квизе, пока есть цели, до которых не дошли.

	`Why:` агент адаптирует занятие под ученика и сокращает материал, а квиз
	ставит сервер и проверяет урок целиком — ученик получал вопрос по теме,
	которой на занятии не было. Тупика отказ не создаёт: повторный
	`report_outcomes` замещает прежний отчёт, значит выход — разобрать
	пропущенное и отчитаться заново.

	`touched` не блокирует: задетая вскользь тема всё-таки разобрана, и
	требовать большего значило бы спорить с педагогикой занятия.
	"""
