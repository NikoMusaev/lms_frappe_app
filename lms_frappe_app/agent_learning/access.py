...
def назначения_ученика(user: str) -> list:
	"""Назначения ученика, прочитанные разом на весь вызов.

	`Why:` за одно `start_lesson` их спрашивали дважды — из списка курсов и из
	политики квиза. Список один и тот же, а стоит он запроса на организации,
	запроса на назначения и по обходу на каждое поимённое. Вызывающий, которому
	нужно и то и другое, читает список здесь и передаёт обеим функциям.
	"""
...
def политика_квиза_для_курса(user: str, course: str, назначения: list | None = None) -> dict:
	...
	`назначения` — уже прочитанный список по всем курсам (`назначения_ученика`);
	без него читаются свои, сразу по этому курсу.
	"""
	if назначения is None:
		назначения = назначения_пользователя(user, course=course)
	курсовые = [назначение for назначение in назначения if назначение.course == course]
...
def _условия_по_курсам(user: str, назначения: list | None = None) -> dict[str, dict]:
