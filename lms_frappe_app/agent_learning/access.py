...
def политика_квиза_для_курса(user: str, course: str) -> dict:
	...
	"""
	курсовые = назначения_пользователя(user, course=course)
	приостановленные = приостановленные_организации(н.organization for н in курсовые)
	организации = [
		назначение.organization
		for назначение in курсовые
		if назначение.organization not in приостановленные
	]
	if not организации:
...
def _условия_по_курсам(user: str) -> dict[str, dict]:
	...
	"""
	назначения = назначения_пользователя(user)
	приостановленные = приостановленные_организации(н.organization for н in назначения)

	условия: dict[str, dict] = {}
	for назначение in назначения:
		приостановлена = назначение.organization in приостановленные
...
def приостановленные_организации(организации: Iterable[str]) -> set[str]:
	"""Какие из организаций списка приостановлены — одним запросом на весь список.

	`Why:` статус спрашивался по разу на каждое назначение, и перечень курсов
	ученика стоил обхода базы на каждое из них. Правило то же, что у
	`организация_приостановлена`, и пропавшая организация так же считается
	приостановленной: действующей её никто не подтвердил.
	"""
	имена = {организация for организация in организации if организация}
	if not имена:
		return set()
	...
	действующие = set(
		frappe.get_all(
			...
			filters={"name": ("in", list(имена)), "status": "Active"},
			pluck="name",
		)
	)
	return имена - действующие


def организация_приостановлена(organization: str) -> bool:
