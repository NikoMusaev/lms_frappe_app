...
Места урока, которые проводит не агент, остаются в тексте **структурными
маркерами** вида ``[[lms:quiz]]`` и ``[[lms:exercise:Название]]``: вид места и
его положение в материале. Прозу по маркеру складывает потребитель контракта —
приложение текстов для агента не порождает.
"""
...
#: Вид места урока, которое проводит не агент. Значение уходит в маркер и
#: потому часть контракта: переименование здесь ломает его разбор снаружи.
КВИЗ = "quiz"
УПРАЖНЕНИЕ = "exercise"

#: Что в названии маркера ломает его разбор: скобка закрывает маркер раньше
#: времени, перевод строки разрывает абзац.
НЕ_В_МАРКЕРЕ = re.compile(r"[\[\]\r\n]+")
...
def маркер(вид: str, название: str = "") -> str:
	"""Отметка места урока, которое проводит не агент.

	Отдаём вид и название, а не готовую фразу: текст для агента складывает
	потребитель контракта. Название очищается от того, что ломает разбор
	маркера, — иначе название из урока разорвало бы его на части.
	"""
	чистое = НЕ_В_МАРКЕРЕ.sub(" ", название or "").strip()
	return f"[[lms:{вид}:{чистое}]]" if чистое else f"[[lms:{вид}]]"
...
def _из_блоков(блоки: list[dict]) -> tuple[str, list[МедиаВложение]]:
	...
	for блок in блоки:
		...
		if тип == "paragraph":
			...
			# Ни идентификатора квиза, ни вопросов в маркере нет: только вид
			# места и его положение в материале.
			куски.append(маркер(КВИЗ))
...
def _из_markdown(body: str) -> tuple[str, list[МедиаВложение]]:
	...
	def заменить(совпадение: re.Match) -> str:
		...
		if имя == "Quiz":
			return маркер(КВИЗ)
		if имя == "Exercise":
			return маркер(УПРАЖНЕНИЕ, аргумент)
		return ""
