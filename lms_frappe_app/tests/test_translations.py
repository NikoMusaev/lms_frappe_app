...
class IntegrationTestTranslations(IntegrationTestCase):
	...
	def test_краевые_пробелы_сохранены(self):
		"""Часть строк — куски предложений из писем; без пробела слова слипнутся."""
		for оригинал, перевод, *_ in _строки():
			self.assertEqual(
				(оригинал[:1] == " ", оригинал[-1:] == " "),
				(перевод[:1] == " ", перевод[-1:] == " "),
				f"разошлись краевые пробелы: {оригинал!r}",
			)
