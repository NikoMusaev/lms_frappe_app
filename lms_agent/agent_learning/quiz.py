...
ВОПРОС_БЕЗ_ЭТАЛОНА = "question_without_answer"
...
def начать_попытку(session: str) -> dict:
	...
	открытая = _открытая_попытка(занятие.student, квиз)
	if открытая:
...
def _открытая_попытка(student: str, квиз: str) -> str | None:
	"""Незавершённая попытка ученика по этому квизу, если она есть."""
	открытые = frappe.get_all(
		"Agent Quiz Attempt",
		filters={"student": student, "quiz": квиз, "status": "In Progress"},
		pluck="name",
...
def _сверить(запись, answer: str) -> tuple[bool, str | None]:
	...
	if запись.type == "Choices":
		...
		if not верные:
			# Иначе пустой ответ совпадёт с пустым множеством эталонов и
			# принесёт балл: кривой контент молча раздавал бы очки.
			raise Отказ(
				ВОПРОС_БЕЗ_ЭТАЛОНА,
				"У вопроса не отмечен ни один верный вариант",
				question=запись.name,
			)
		пояснение = " ".join(
